...
class MyGETA(BaseHybridSparseOptimizer):
    """
    ...
    def __init__(
        ...
        self.current_prune_groups = 0
        self.prune_groups_flag = False

        super(MyGETA, self).__init__(
            params=params,
            variant=variant,
            lr=lr,
        ...
        # self.active_num_redundant_groups = [1,1,1]
        self.logger.info(
    ...
    def step(self, loss=None, closure=None):
        ...
        核心优化步骤函数。
        修改点：
        1. 删除了 start_projection_step 相关的仅投影位宽调整逻辑。
        2. 在联合阶段的周期触发点加入 adaptive_bit_reduction。
        """
        ...
        if (
            ...
            if (
                ...
                period = self.curr_pruning_period
                if period >= len(self.active_num_redundant_groups):
                    self.logger.warning(f"Period {period} has no pruning budget, skip identification")

                self.logger.info(f"Step: {self.num_steps} - 执行周期性调整：归位、分数计算、自适应位宽缩减")

                # 1. 提交索引
                self.commit_redundant_idxes()
                # 2. 计算重要性分数
                self.compute_importance_scores()
                # 3. [新增集成] 自动调整全局最大位宽 (假设阈值为 0.7)
                self.adaptive_bit_reduction(threshold=0.7)
                # 4. 识别新冗余组
        ...
        for group in self.param_groups:
            ...
            # 情况2：联合剪枝与量化阶段
                ...
                T_duration = self.pruning_period_duration
                t_val = ((self.num_steps - self.start_pruning_step) % T_duration) + 1
                ...
                gamma, d_quant = self.compute_gamma_d(group, active_redundant_idxes, [self.min_bit_wt, self.max_bit_wt])

                for i, p_name in enumerate(group["p_names"]):
                    if "d_quant_wt" in p_name:
                        with torch.no_grad():
                ...
                for p_name, p, p_transform in zip(group["p_names"], group["params"], group["p_transform"]):
                    ...
                    if p_transform != TensorTransform.NO_PRUNE:
                        if is_quantize:
                            p.data[active_redundant_idxes] -= gamma * quantize_weight.data[active_redundant_idxes]
                        else:
                            p.data[active_redundant_idxes] -= gamma * p.data[active_redundant_idxes]

                    # B. 权重层更新及加噪
                    if all(k not in p_name for k in ["d_quant", "t_quant", "q_m"]):
                ...
                for ng_id, offset in group["auxiliary_ngs"]:
                    ...
                    for aux_p in self.auxiliary_param_groups[ng_id]["params"]:
                        if aux_p.grad is None: continue
                        aux_p.data[active_redundant_aux_idxes, ...] *= (T_duration - t - 1.0) / (T_duration - t)
    ...
    def set_pruning_budget_this_period(self, rate_to_prune, period = None):
        """
    外部控制：当前/指定 period 要剪多少组
    Args:
        num_groups_to_prune: int, 本 period 要剪的参数比例
        period: int or None, None 表示当前 period
    """
        num_groups_to_prune = max(1, int(self.total_num_groups * rate_to_prune))
        if period is None:
            period = self.curr_pruning_period
            print(period)

        if period >= self.pruning_periods:
            old_pruning_periods = self.pruning_periods
            self.pruning_periods = period + 1  # 总周期数 = 最大period + 1（因为period从0开始）
            self.logger.info(
                f"Pruning periods extended: {old_pruning_periods} → {self.pruning_periods} "
                f"(to match budget period {period})"
            )

        if period < len(self.active_num_redundant_groups):
            old = self.active_num_redundant_groups[period]
            self.active_num_redundant_groups[period] = num_groups_to_prune
    ...
    def _get_cosine_noise(self, param, lr, t, T):
        """
        计算并返回噪声张量
        强度 = lr * sqrt(1 - cos((T - t) * pi / (T - 1)))
        """
        if T <= 1:
            return torch.zeros_like(param)

        # 将相位改为 (T - t) * pi / (T - 1)
        # 当 t=1 (开始) 时，相位接近 pi，cos 为 -1，强度最大
        # 当 t=T (结束) 时，相位为 0，cos 为 1，强度为 0
        angle = ((T - t) * math.pi) / (T - 1)
        scale = math.sqrt(1 - math.cos(angle))
        noise_std = lr * scale

        return torch.randn_like(param) * noise_std

    def adaptive_bit_reduction(self, threshold=0.5):
        """
        根据各层位宽分布情况，自适应降低全局最大位宽上限。
        Args:
            threshold (float): 触发减一操作的比例阈值 (0.0 - 1.0)。
                               即当超过该比例的层都已经“进化”到更低位宽时，才调低上限。
        """
        total_layers = 0
        wt_lower_than_max = 0
        act_lower_than_max = 0

        # 1. 遍历所有参数组，收集当前所有层的真实位宽
        for group in self.param_groups:
            # 使用类已有的 get_bitwidth_dict 方法
            bit_dict = self.get_bitwidth_dict(group)
            ...
            for layer_name, bits in bit_dict.items():
                ...
                # 检查权重的位宽
                if 'weight' in bits:
                    if bits['weight'] < self.max_bit_wt:
                        wt_lower_than_max += 1

                # 检查激活值的位宽
                if 'activation' in bits:
                    if bits['activation'] < self.max_bit_act:
                        act_lower_than_max += 1

        if total_layers == 0:
            return

        # 2. 计算比例并执行调整
        wt_ratio = wt_lower_than_max / total_layers
        act_ratio = act_lower_than_max / total_layers

        # 如果大于 threshold 的层都已经降低了位宽，则全局上限减一
        if wt_ratio > threshold:
            old_max_wt = self.max_bit_wt
            self.max_bit_wt = max(self.min_bit_wt, self.max_bit_wt - 1)
            self.logger.info(
                f"Adaptive Weight Bit Reduction: {old_max_wt} -> {self.max_bit_wt} (Ratio: {wt_ratio:.2%})")

        if act_ratio > threshold:
            old_max_act = self.max_bit_act
            self.max_bit_act = max(self.min_bit_act, self.max_bit_act - 1)
            self.logger.info(
                f"Adaptive Activation Bit Reduction: {old_max_act} -> {self.max_bit_act} (Ratio: {act_ratio:.2%})")
