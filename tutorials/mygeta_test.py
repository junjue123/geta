...
if __name__=="__main__":
    ...
    pruning_ratios = generate_smooth_pruning_ratios(0.7,pruning_epochs)
    ...
    # 使用模型的KB大小
    compressed_model_size = os.stat(oto.full_group_sparse_model_path).st_size/ (1024 ** 1)
    target_model_size = 256 #KB

    pruner = DynamicPruner(
        original_size=compressed_model_size,
        target_size=target_model_size,
        prune_rate_range=(0.01, 0.05),
        smooth_factor=3
    )
    ...
    for epoch in range(max_epoch):
        ...
        oto.construct_subnet(out_dir='temp')
        # 使用模型的KB大小
        compressed_model_size = os.stat(oto.compressed_model_path).st_size/ (1024 ** 1)
